...
export function reduceDungeonCrawlRolls(event: any): void {
  const earlyStages = ['dungeoncrawl:chests/stage_1', 'dungeoncrawl:chests/stage_2'];
  const midStages = ['dungeoncrawl:chests/stage_3', 'dungeoncrawl:chests/stage_4', 'dungeoncrawl:chests/stage_5'];
  const treasures = ['dungeoncrawl:chests/treasure', 'dungeoncrawl:chests/secret_room'];

  for (const id of earlyStages) {
    event.getLootTable(id).firstPool((pool: any) => pool.rolls([2, 4]));
  }
  for (const id of [...midStages, ...treasures]) {
